 ...
 */
/**
 * Kill orphaned processes (non-gateway) to prevent sandbox process exhaustion.
 * This runs during the cron job to keep the sandbox clean.
 */
async function cleanupOrphanedProcesses(sandbox: ReturnType<typeof getSandbox>): Promise<void> {
  try {
    const processes = await sandbox.listProcesses();
    if (processes.length <= 5) return;

    console.log('[cleanup] Found', processes.length, 'processes, cleaning orphans...');
    let killed = 0;
...
async function scheduled(
  ...
  // Clean up orphaned processes on every cron run
