 ...
 *
 * This will:
 * 1. Restore files from R2 backup if available
...
export async function ensureMoltbotGateway(sandbox: Sandbox, env: MoltbotEnv): Promise<Process> {
  // Check if gateway is already running or starting (fast path — no mount needed)
  ...
  // Restore files from R2 directly into the container (replaces flaky s3fs mount)
  ...
  // Write env vars to a file in the container since startProcess({ env }) doesn't
  // reliably pass env vars to the process. The start script sources this file.
  if (Object.keys(envVars).length > 0) {
    const envLines = Object.entries(envVars)
      .map(([k, v]) => `export ${k}=${JSON.stringify(v)}`)
      .join('\n');
    try {
      await sandbox.writeFile('/tmp/container-env.sh', envLines);
      console.log('Wrote env vars file with', Object.keys(envVars).length, 'vars');
    } catch (err) {
      console.error('Failed to write env vars file:', err);
    }
  }
  ...
  try {
    ...
    // Note: process.getLogs() intentionally NOT called here — it hangs indefinitely (Cloudflare SDK bug)
  } catch (e) {
    console.error('[Gateway] waitForPort failed:', e);
    throw new Error(`OpenClaw gateway failed to start within ${STARTUP_TIMEOUT_MS}ms`, { cause: e });
  }
